...
st.title("🛍️🌐 Predicting Delivery Times Using A Real, Commercial E-Commerce Dataset 💰")
...
with tab1: 
    ...
    col2.metric(label="Number of Tables Used", value=8)
    col3.metric(label="Average Processing Time for The Data & Modeling Pipeline Combined", value='<5 Min')
...
with tab2:
    ...
    st.divider()
    with st.container():
        st.subheader("Average Delivery Duration: Actual vs Predicted Over Time (Line)")
...
with tab3: 
    ...
    st.image(image, caption='ERD of Olist Ecommerce Dataset')
